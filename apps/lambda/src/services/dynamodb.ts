...
export const dynamodbService = {
  getReviewId: (platform: string, appId: string, reviewId: string): string => {
  ...
  reviewExists: async (reviewIdentifier: string): Promise<boolean> => {
    try {
  ...
  getAllReviews: async (limit: number = 1000): Promise<ReviewItem[]> => {
    try {
      const result = await dynamoClient.send(
        new ScanCommand({
          TableName: TABLE_NAME,
