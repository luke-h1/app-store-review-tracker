...
resource "aws_apigatewayv2_domain_name" "domain_name" {
  domain_name = var.env == "production" ? "reviewer.${var.root_domain}" : "reviewer-${var.env}.${var.root_domain}"
...
resource "aws_apigatewayv2_route" "lambda_route_proxy" {
  ...
  operation_name     = "get reviews"
  authorizer_id      = aws_apigatewayv2_authorizer.api_key.id
  authorization_type = "CUSTOM"
}

resource "aws_apigatewayv2_route" "lambda_route_analytics" {
  api_id             = aws_apigatewayv2_api.lambda.id
  target             = "integrations/${aws_apigatewayv2_integration.lambda.id}"
  route_key          = "GET /api/analytics"
  operation_name     = "get analytics"
  authorizer_id      = aws_apigatewayv2_authorizer.api_key.id
  authorization_type = "CUSTOM"
}
